...
import { fetchLogs, postLog, deleteLog } from "./api";
...
export default function App() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setLogs((prev) => [response.log, ...prev]); // ← musi być response.log z id
      setTestName("");
      setResult("pass");
    } catch (err) {
  ...
  return (
    <div className="p-6">
      ...
      <form onSubmit={handleSubmit} className="mt-6 p-4 border rounded">
        ...
        {sendError && <p className="text-red-600 mt-2">{sendError}</p>}
      </form>
    </div>
  );
}